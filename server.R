...
shinyServer(function(input, output, session) {
  ...
  output$currentTime <- renderText({invalidateLater(1000, session) 
    paste("Current time and date is: ", "TZ:", Sys.timezone(), "local date and time is:", Sys.time())})
  ...
  observe({
    ...
    tweets_result = searchTwitter("harvey")
    for (tweet in tweets_result){
      print(paste(tweet$screenName, ":", tweet$text))
      
      vector_users <- c(vector_users, as.character(tweet$screenName));
      
      if (grepl("love", tweet$text, ignore.case = TRUE)  | grepl("Wonderful", tweet$text, ignore.case = TRUE) | grepl("Awesome", tweet$text, ignore.case = TRUE)){
        count_positive = count_positive + 1
        
        vector_sentiments <- c(vector_sentiments, "Positive")
        positive_text <- c(positive_text, as.character(tweet$text))
        
      } else if (grepl("danger", tweet$text, ignore.case = TRUE) | grepl("flood", tweet$text, ignore.case = TRUE)| grepl("hurricane", tweet$text, ignore.case = TRUE)) { 
    ...
    output$distPlot <- renderPlot({
      ...
      if (length(positive_text) > 0){
        ...
        output$positive_wordcloud <- renderPlot({ wordcloud(paste(positive_text, collapse=" "), min.freq = 0, random.color=TRUE, max.words=10 ,colors=brewer.pal(8, "Dark2")) }) 
      }
      
      if (length(negative_text) > 0) {
        
        output$negative_wordcloud <- renderPlot({ wordcloud(paste(negative_text, collapse=" "), random.color=TRUE, min.freq = 0, max.words=10 ,colors=brewer.pal(8,"Set3")) }) 
      }
      
      
      if (length(neutral_text) > 0){
        
        output$neutral_wordcloud <- renderPlot({ wordcloud(paste(neutral_text, collapse=" "), min.freq = 0, random.color=TRUE , max.words=10 ,colors=brewer.pal(8, "Dark2")) }) 
      }
